...
@ASerializable("FPColorMinerPlayerInteractionMode")
export class FPColorMinerPlayerInteractionMode extends MainAppInteractionMode {
    ...
    camOffsets: Vec3 = new Vec3(0,0,.35);
    targetOffsets: Vec3 = new Vec3(10,10,0);
    ...
    reset() {
        ...
        let camLoc = this.player.position.clone();
        let camTarget = this.player.position.clone();
        let rot = this.player.transform.rotation.getAxisAndAngle();
        camLoc.z += (this.camOffsets.z);
        camTarget.z = this.targetOffsets.z;
        if (rot.axis.isEqualTo(Vec3.UnitZ())){
            camLoc.y += (this.camOffsets.y) * Math.cos(Math.PI*2-rot.angle);
            camLoc.x += (this.camOffsets.x) * Math.sin(Math.PI*2-rot.angle);

            camTarget.y += (this.targetOffsets.y) * Math.cos(Math.PI*2-rot.angle);
            camTarget.x -= (this.targetOffsets.x) * Math.sin(Math.PI*2-rot.angle);
        }
        else{
            camLoc.y += (this.camOffsets.y) * Math.cos(rot.angle);
            camLoc.x += (this.camOffsets.x) * Math.sin(rot.angle);

            camTarget.y += (this.targetOffsets.y) * Math.cos(rot.angle);
            camTarget.x -= (this.targetOffsets.x) * Math.sin(rot.angle);
        }        
    ...
    onDragMove(event: AInteractionEvent, interaction: ADragInteraction): void {
        ...
        this.targetOffsets = qY.appliedTo(this.targetOffsets);
        // console.log(this.targetOffsets.toString());
        this.reset()
